...
function automaticSync() {
  ...
  customLog(
    `✅ Sincronización automática configurada para ejecutarse cada ${syncInterval} minutos de ${startHour}:00 a ${endHour}:00 (${timezone})`
  );
    ...
    async () => {
      ...
      try {
        ...
        if (result && result.upsertedCount !== undefined) {
          customLog(
            `✅ [${currentTime}] Sincronización automática completada: ${result.upsertedCount} códigos añadidos o actualizados.`
          );
        } else {
